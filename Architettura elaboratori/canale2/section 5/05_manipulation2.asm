...
.eqv $source, $t1								# Conterr� indirizzo del valore passato in input
.eqv $dest, $t2									# Conterr� il valore eventualmente modificato dell'input
...
loop:
	...
	bnez $stat1, capitalize 				# Se non � zero allora il valore � compreso tra 'a' e 'z', quindi richiama capitalize che si occuper� del controllo e della trasformazione 
	...
	sle $stat2, $bit, 'Z'
	and $stat1, $stat1, $stat2
	bnez $stat1, minimize					# Se il valore precendente non � lo spazio allora ci troviamo all'interno di una frase 
...
capitalize:											
	bne $prev, ' ', normal					#Se il precedente non � uno spazio la lettera non dece essere trasformata e si prosegue con il semblice salvataggio
	subi $bit, $bit, 32
...
minimize:
	beq $prev, ' ', normal					#Se il precedente non � uno spazio la lettera non deve essere trasformata e si prosegue con il semblice salvataggio
	addi $bit, $bit, 32
	sb $bit, ($dest)								# Salva il bit nel vettore di destinazione
